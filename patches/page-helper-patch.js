...
module.exports = {
   ...
   *
   * PATCHED: quote YAML values containing special characters (colons, quotes, hashes)
   * to prevent js-yaml parse failures on git sync round-trip.
   * Without this fix, page titles like "My Page: Subtitle" produce invalid YAML
   * that breaks re-import.
   * See: https://github.com/requarks/wiki/discussions/6818
   */
  injectPageMetadata(page) {
    ...
    switch (page.contentType) {
      case 'markdown':
        return '---\n' + meta.map(mt => {
          const val = String(mt[1])
          if (val.includes(':') || val.includes('"') || val.includes('#') || val.includes("'")) {
            return `${mt[0]}: "${val.replace(/"/g, '\\"')}"`
          }
          return `${mt[0]}: ${val}`
        }).join('\n') + '\n---\n\n' + page.content
      case 'html':
